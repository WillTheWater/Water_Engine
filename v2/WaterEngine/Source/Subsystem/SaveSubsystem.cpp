...
namespace we
{
	...
	string SaveSubsystem::GetSavePath() const
	{
#ifdef _WIN32
	...
	bool SaveSubsystem::Has(stringView Key) const
	{
		return SaveData.contains(Key);
	}

	void SaveSubsystem::Erase(stringView Key)
	{
		SaveData.erase(Key);
	}

	void SaveSubsystem::Clear()
	{
		SaveData.clear();
	}
}
