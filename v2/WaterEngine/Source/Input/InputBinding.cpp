...
namespace we
{
    ...
    optional<int> Input::LogicToHardware(GamepadButton Button, int HardwareID)
    {
        const auto& Mapping = InputMapping::GetGamepadMapping(HardwareID);
        const int Value = Mapping[(int)Button];
        return (Value != -1) ? optional(Value) : std::nullopt;
    }
